...
export function ResendEmailVerificationCodeForm() {
  ...
  useEffect(() => {
    const cooldownEnd = localStorage.getItem('resendCooldownEndVE');
    const lastAttemptTime = localStorage.getItem('lastResendAttemptVE');
    const attemptCount = localStorage.getItem('resendAttemptCountVE');
    ...
    if (lastAttemptTime) {
      ...
      if (timeSinceLastAttempt > 5 * 60 * 1000) {
        // 5 minutes
        localStorage.removeItem('resendAttemptCountVE');
        localStorage.removeItem('lastResendAttemptVE');
      }
    }
  ...
  useEffect(() => {
    if (timeLeft <= 0) {
      localStorage.removeItem('resendCooldownEndVE');
      return;
    }
  ...
  const handleSubmit = async () => {
    const attemptCount = Number.parseInt(
      localStorage.getItem('resendAttemptCountVE') || '0',
    );
    ...
    localStorage.setItem('resendCooldownEndVE', cooldownEnd.toString());
    localStorage.setItem('resendAttemptCountVE', newAttemptCount.toString());
    localStorage.setItem('lastResendAttemptVE', Date.now().toString());
