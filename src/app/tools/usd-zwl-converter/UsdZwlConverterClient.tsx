...
export default function UsdZwlConverterClient() {
  ...
  const [parallelRateSource, setParallelRateSource] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchRates = async () => {
      try {
        ...
        const response = await fetch('https://zimrate.tyganeutronics.com/api/v1');
        if (!response.ok) {
        ...
        const rbzRateEntry = data.USD.find((rate: any) => rate.currency === 'RBZ');
        if (rbzRateEntry) {
          setOfficialRate(rbzRateEntry.rate);
        } else {
          setError(prev => (prev ? prev + ', ' : '') + 'Official rate not found.');
        }

        // Find Parallel Rate (OMIR or another suitable one like ZWG CASH)
        const omirRateEntry = data.USD.find((rate: any) => rate.currency === 'OMIR');
         const zwgCashRateEntry = data.USD.find((rate: any) => rate.name === 'ZWG CASH (zimpricecheck)');
        ...
        if (omirRateEntry) {
          ...
          setError(prev => (prev ? prev + ', ' : '') + 'Parallel rate not found.');
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Convert between USD and ZiG using real-time rates</p>
          </div>
        </div>
      </div>
        ...
        <CardContent>
          {loadingRates ? (
            ...
            <p className="text-destructive">Error: {error}</p>
             ...
             <p className="text-destructive">Could not load any rates. Please try again later.</p>
          ) : (
            <div className="grid gap-4">
              ...
              {officialRate !== null && (
                <div className="mt-2 text-sm text-muted-foreground">
                  ...
                  <p>Official Conversion: {usdAmount || '0'} USD ≈ <span className="font-semibold text-foreground">{officialZigAmount || '0.00'}</span> ZiG</p>
                </div>
              )}
              ...
              {parallelRate !== null && (
                <div className="mt-2 text-sm text-muted-foreground">
                   ...
                   <p>Parallel Rate ({parallelRateSource || 'N/A'}): 1 USD ≈ {parallelRate.toFixed(2)} ZiG</p>
                  <p>Parallel Conversion: {usdAmount || '0'} USD ≈ <span className="font-semibold text-foreground">{parallelZigAmount || '0.00'}</span> ZiG</p>
