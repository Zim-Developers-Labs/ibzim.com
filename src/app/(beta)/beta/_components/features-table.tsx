...
import { FileText, Star, Bug, ChevronDown, MessageCircle } from 'lucide-react';
...
const pendingIssues: Record<
  string,
  { id: string; title: string; status: string }[]
> = {
  'School Picker Tables': [
    { id: 'SPT-001', title: 'Filter not working on mobile', status: 'Open' },
    {
      id: 'SPT-002',
      title: 'Sorting resets on page change',
      status: 'In Review',
    },
  ],
  Premium: [
    { id: 'PRM-001', title: 'Payment confirmation delay', status: 'Open' },
  ],
  'Articles Blog': [
    {
      id: 'ART-001',
      title: 'Images not loading in preview',
      status: 'In Review',
    },
    { id: 'ART-002', title: 'Comment count incorrect', status: 'Open' },
  ],
  'IBZIM Docs': [
    { id: 'DOC-001', title: 'Search results not accurate', status: 'Open' },
  ],
};
...
function FeatureRow({
  ...
  const issues = pendingIssues[feature.name] || [];
          ...
          <div className="space-y-4 rounded-lg border border-zinc-200 bg-white p-4">
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor={`title-${feature.name}`} className="text-xs">
                  Issue Title
                </Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label htmlFor={`desc-${feature.name}`} className="text-xs">
                  Description
                </Label>
                <Textarea
                  id={`desc-${feature.name}`}
