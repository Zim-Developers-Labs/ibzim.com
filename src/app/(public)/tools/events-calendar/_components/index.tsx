...
import { EventType, OrganizerProfileType } from '@/lib/server/db/schema';
...
export default function CalendarComponents({
  ...
  organizer: OrganizerProfileType | null;
  dbEvents: EventType[];
}) {
  const { user } = useUser();
  ...
  const getEventIcon = (category: EventType['category']) => {
    switch (category) {
      case 'holiday':
  ...
  const getCategoryColorForMini = (category: EventType['category']) => {
    switch (category) {
      case 'holiday':
  ...
  const [selectedEventOrganizer, setSelectedEventOrganizer] =
    useState<OrganizerProfileType | null>(null);
