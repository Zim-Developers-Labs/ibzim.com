'use client';

import { Tabs, TabsList, TabsTrigger } from '@/components/ui/tabs';

export default function PremiumHero({
  subType,
  setSubType,
}: {
  subType: string;
  setSubType: (subType: string) => void;
}) {
  return (
    <section className="relative overflow-hidden bg-zinc-900 px-4 pt-28 pb-48 text-white sm:px-6">
      <svg
        viewBox="0 0 1208 1024"
        aria-hidden="true"
        className="absolute -bottom-48 left-1/2 z-0 h-[64rem] -translate-x-1/2 translate-y-1/2 [mask-image:radial-gradient(closest-side,white,transparent)] opacity-20 lg:-top-0 lg:bottom-auto lg:translate-y-0"
      >
        <ellipse
          cx={604}
          cy={512}
          rx={604}
          ry={512}
          fill="url(#d25c25d4-6d43-4bf9-b9ac-1842a30a4867)"
        />
        <defs>
          <radialGradient id="d25c25d4-6d43-4bf9-b9ac-1842a30a4867">
            <stop stopColor="#EAB308" />
            <stop offset={1} stopColor="#FF5A5A" />
          </radialGradient>
        </defs>
      </svg>
      <h1 className="mb-4">
        <span className="from-secondaryColor via-primaryColor to-secondaryColor block bg-gradient-to-t bg-clip-text text-center tracking-widest text-transparent">
          IBZIM PREMIUM
        </span>
        <span className="block text-center text-4xl tracking-wide">
          Knowledge is Power
        </span>
      </h1>
      <p className="mx-auto mb-20 max-w-lg text-center font-light">
        Invest in your knowledge with unlimited IBZIM content access and
        exclusive premium features.
      </p>
      <div className="absolute left-1/2 mx-auto mt-12 w-fit -translate-x-1/2 -translate-y-20">
        <Tabs value={subType} onValueChange={setSubType}>
          <TabsList>
            <TabsTrigger
              value="personal"
              className="data-[state=active]:bg-primaryColor cursor-pointer px-4 data-[state=active]:text-white"
            >
              Personal
            </TabsTrigger>
            <TabsTrigger
              value="business"
              className="data-[state=active]:bg-primaryColor cursor-pointer px-4 data-[state=active]:text-white"
            >
              Business
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      <div className="relative z-10 mx-auto w-fit pt-10 text-center text-sm">
        <span className="font-light">
          {subType === 'business' ? (
            <span>
              Switch to personal plans for{' '}
              <span className="text-primaryColor">
                quality content consumption
              </span>
              .
            </span>
          ) : (
            <span>
              Switch to business plans for{' '}
              <span className="text-primaryColor">
                income generation benefits
              </span>
              .
            </span>
          )}
        </span>{' '}
      </div>
    </section>
  );
}
