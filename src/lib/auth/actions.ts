...
export async function signup(
  ...
  return redirect(`${Paths.VerifyEmail}?callbackUrl=${callbackUrl}`);
}
...
export async function completeProfile(
  _: any,
  formData: FormData,
): Promise<ActionResponse<OnboardingInput>> {
  const obj = Object.fromEntries(formData.entries());

  const parsed = onboardingSchema.safeParse(obj);
  if (!parsed.success) {
    const err = parsed.error.flatten();
    return {
      fieldError: {
  ...
  const { country, city, userId } = parsed.data;
...
export async function verifyEmail(
  ...
  const callbackUrl = formData.get('callbackUrl') as string;
  redirect(`${Paths.Onboarding}?callbackUrl=${callbackUrl}`);
}
